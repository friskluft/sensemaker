	...
	bool process_whole_slide (const std::string& intens_fpath, ImageLoader & imlo, LR & fakeroi)
	{
		...
		// Timing block (image scanning)
		{
			...
			{ STOPWATCH("Image scan2a/scan2a/s2a/#aabbcc", "\t=");
				// Phase 1: gather ROI metrics
				VERBOSLVL2(std::cout << "Gathering ROI metrics\n");
				bool okGather = gather_wholeslide_metrics (intens_fpath, imlo, fakeroi);	//????????????????? gatherRoisMetrics(intens_fpath, intens_fpath, imlo);
				if (!okGather)
				{
					std::string msg = "Error in gatherRoisMetrics()\n";
					std::cerr << msg;
			...
			{ STOPWATCH("Image scan2b/scan2b/s2b/#aabbcc", "\t=");
			// Allocate the feature value buffer
			fakeroi.initialize_fvals();
			}
			...
			// Examine ROI's memory footprint
			if (size_t roiFootprint = fakeroi.get_ram_footprint_estimate(),
				ramLim = theEnvironment.get_ram_limit();
				roiFootprint >= ramLim)
			{
				VERBOSLVL2(
					...
					<< " (S=" << fakeroi.aux_area
					<< " W=" << fakeroi.aabb.get_width()
					<< " H=" << fakeroi.aabb.get_height()
					<< " px footprint=" << roiFootprint << " b"
					<< ")\n"
				);

				std::cout << intens_fpath << ": slide is non-trivial \n";
				return false;
			}
			} //- stopwatch
		}
