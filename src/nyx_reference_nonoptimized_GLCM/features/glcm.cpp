	...
	const SimpleMatrix<uint8_t>& grays)
{
	...
	for (int row = PGM_MAXMAXVAL; row >= 0; --row)
		tone_LUT[row] = -1;
	for (int col = 0; col < ncols; ++col)		//OPT	for (int row = nrows - 1; row >= 0; --row)
		for (int row = nrows - 1; row >= 0; --row)	//OPT	for (int col = 0; col < ncols; ++col)
	...
	int tone_count)
{
	...
	for (col = 0; col < cols; ++col)	//OPT	for (row = 0; row < rows; ++row)
		for (row = 0; row < rows; ++row)	//OPT	for (col = 0; col < cols; ++col)
		{
