...
int mine_segment_relations (const std::string& label_dir, const std::string& file_pattern, const std::string& channel_signature, const int parent_channel, const int child_channel)
{
	...
	if (segFiles.size() == 0)
	{
		std::cout << "No label files to process\n";
		return 2;
	}
	...
	for (auto& segf : segFiles)
	{
		...
		if (lenOfNumeric <= 0)
		{
			std::cout << "Cannot find the numeric part in channel '" << channel << "'\n";
			return 1;
		}
	...
	for (auto& parStemInfo : Stems)
	{
		...
		if (!ok)
		{
			std::cout << "Error finding hierarchy based on files " << ssParFname.str() << " as parent and " << ssChiFname.str() << " as children. Quitting\n";
			return 1;
		}
		...
		if (!ok)
		{
			std::cout << "Cannot produce the output: somethig is wrong with data. Quitting\n";
			return 2;
		}
	}
